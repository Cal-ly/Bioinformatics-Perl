README:  Gene                       Last modified:  April 8, 2013
...
You may subscribe to the Gene News RSS feed here:
            http://www.ncbi.nlm.nih.gov/feed/rss.cgi?ChanKey=genenews
...
I.     DATA directory
II.    DATA directory, ASN_OLD subdirectory (no longer updated beginning
                                         August 1, 2005)
III.   DATA directory, ASN_BINARY subdirectory
IV.    DATA directory, GENE_INFO subdirectory
...
---------------------------------------------------------------------------
           ...
           in the gene2ensembl FTP file (see previous item).
...
---------------------------------------------------------------------------
           ...
           Because this file is updated daily, the RefSeq subset does not 
             ...
             To identify the annotation release/build to which the 
             genomic RefSeqs belong, please refer to the species-specific
             README_CURRENT_RELEASE or README_CURRENT_BUILD file in the genomes ftp site:
...
===========================================================================
gene2unigene                                    recalculated daily
---------------------------------------------------------------------------
           This file can be considered as the logical equivalent of
...
===========================================================================
mim2gene_partial                                recalculated daily
---------------------------------------------------------------------------
           ...
           NOTE:  this file was renamed to make the point that Gene will
           no longer guarantee that this file is comprehensive because
           NIH does not have a license from JHU for OMIM
           ...
           GeneID
              current values are 
                ...
                'phenotype' the MIM number associated with a disease
                            that is associate with a gene
see also:
http://www.ncbi.nlm.nih.gov/bookshelf/br.fcgi?book=helpgene&part=genefaq#genefaq.Entrez_Gene_and_OMIM


===========================================================================
mim2gene_medgen                               being developed, will be reported daily
---------------------------------------------------------------------------
           report of the relationship between MIM numbers (OMIM), 
...
GeneID:
           the current unique identifier for a gene
           if there is no GeneID but there is a phenotype, the GeneID column has a -1
           if NCBI has no record of this MIM number in its databases yet, there is a '-' provided
           in the GeneID column
type:
           type of relationship between the MIM number and the 
           GeneID
              current values are 
...
source:    This value is provided only when there is a report of a relationship
           between a MIM number that is a phenotype, and a GeneID.
           The current expected values are GeneMap (from OMIM), GeneReviews, 
           GeneTests, and NCBI.

MedGenCUI  The accession assigned by MedGen to this phenotype.  If the accession starts
...
see also:
http://www.ncbi.nlm.nih.gov/books/NBK3841/#EntrezGene.Words_Excluded_From_Queries

===========================================================================
===========================================================================
II.    Files in the DATA/ASN_OLD directory
---------------------------------------------------------------------------
This directory and all its subdirectories contain complete extractions from 
Entrez Gene in compressed ASN.1 text format. Each unit record is concatenated;
the ASN.1 is not provided as a set as in the DATA/ASN_BINARY directory.

This format of the record has been phased out. The same format can be
created from the files in the ASN_BINARY directory, using gene2xml with the
 -x option. 

On August 1, 2005, updates to this directory ceased.
===========================================================================
===========================================================================
All_Data.gz                        all records

Archea_Bacteria                    directory for Genes from Archaea and 
                                      Bacteria

    All_Archaea_Bacteria.gz        all records from Archaea and Bacteria
    Archaea.gz                     Archaea only
    Bacteria.gz                    Bacteria only

Fungi                              directory for Genes from Fungi

    All_Fungi.gz                   all records from Fungi, including 
                                       organelles

    Ascomycota.gz                  Ascomycota only
    Microsporidia.gz               Microsporidia only
    Saccharomyces_cerevisiae.gz    Saccharomyces cerevisiae only

Invertebrates                      directory for genes from invertebrates

    All_Invertebrates.gz           all records from invertebrates
    Anopheles_gambiae.gz           Anopheles gambiae only
    Caenorhabditis_elegans.gz      Caenorhabditis elegans only
    Drosophila_melanogaster.gz     Drosophila melanogaster only
    Plasmodium_falciparum.gz       Plasmodium falciparum only

Mammalia                           directory for genes from mammals

    All_Mammalia.gz                all records from mammals, including 
                                     organelles
    Bos_taurus.gz                  Bos taurus only
    Canis_familiaris.gz            Canis familiaris only
    Homo_sapiens.gz                Homo sapiens only
    Mus_musculus.gz                Mus musculus only
    Pan_troglodytes.gz             Pan troglodytes only
    Rattus_norvegicus.gz           Rattus norvegicus only
    Sus_scrofa.gz                  Sus scrofa only

Non-mammalian_vertebrates          directory for non-mammalian vertebrates

    All_Non-mammalian_vertebrates.gz
                                   all records from non-mammalian vertebrates
    Danio_rerio.gz                 Danio rerio only
    Gallus_gallus.gz               Gallus gallus only
    Xenopus_laevis.gz              Xenopus laevis only
    Xenopus_tropicalis.gz          Xenopus tropicalis only

Plants                             directory for plants

    All_Plants.gz                  all records from plants
    Arabidopsis_thaliana.gz        Arabidopsis thaliana only
    Oryza_sativa.gz                Oryza sativa only

Viruses                            directory for viruses

    All_Viruses.gz                 all records from viruses
    Retroviridae.gz                Retroviridae only
    dsDNA_viruses,_no_RNA_stage.gz dsDNA_viruses, no RNA stage only
    dsRNA_viruses.gz               dsRNA_viruses only
    ssDNA_viruses.gz               ssDNA_viruses only
    ssRNA_negative-strand_viruses.gz
                                   ssRNA negative-strand viruses only
    ssRNA_positive-strand_viruses,_no_DNA_stage.gz
                                   ssRNA positive-strand viruses, no DNA stage only


===========================================================================
===========================================================================
III.    Files in the DATA/ASN_BINARY directory
---------------------------------------------------------------------------
This directory and all its subdirectories contain complete extractions from 
...
Viruses                            directory for viruses
    ...
    dsDNA_viruses,_no_RNA_stage.ags.gz 
                                   dsDNA_viruses, no RNA stage only
    dsRNA_viruses.ags.gz           dsRNA_viruses only
    ssDNA_viruses.ags.gz           ssDNA_viruses only
    ssRNA_negative-strand_viruses.ags.gz
                                   ssRNA negative-strand viruses only
    ssRNA_positive-strand_viruses,_no_DNA_stage.ags.gz
                                   ssRNA positive-strand viruses, no DNA stage only


===========================================================================
===========================================================================
IV.    Files in the DATA/GENE_INFO directory
---------------------------------------------------------------------------
...
---------------------------------------------------------------------------
            ...
            hiv_interactions.gz, described above.
...
ftp://ftp.ncbi.nlm.nih.gov/asn1-converters/documentation/gene2xml.txt
...
As part of the genome annotation process, tab-delimited files are created that
give the position of key features in both contig (RefSeq accessions of the 
format NW_ or NT_) and chromosome coordinates, if applicable.

Start at 
    ftp://ftp.ncbi.nih.gov/genomes/

and find the genome-specific directories of interest. 

Within each, click on maps, then mapview, then the folder for the current
build.  In that directory you should find the file seq_gene.md. The gene lines
in this file give the ranges for the gene in chromosome (as applicable) and
contig coordinates. For example, a command like
...
chrStart, chrEnd, and orientation refer to the positions on chromosome.
cnt_start, cnt_stop, and cnt_orient refer to positions on the contigs.
Both are 1-based.
...
There is a GFF directory for each genome, e.g. 

ftp://ftp.ncbi.nih.gov/genomes/H_sapiens/GFF/

The most recent annotation is provided for in GFF format for each assembly for that species, 
for both top-level sequences and scaffolds.  

Complete details about NCBI's GFF files are provided in the README file in each 
genome-specific directory.